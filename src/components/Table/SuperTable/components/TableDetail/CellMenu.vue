...
<template>
  <div v-if="isMenu" :style="{ top: mouseY + 'px', left: mouseX + 'px' }" class="row-detail" ref="menuRef">
    <div class="menu">
      <div class="menu-item" v-for="item in menus" :key="item.value" @click="selectEvent(item.value)">
...
import { insertRowAbove, insertRowBelow, deleteRow,addSubRow,createNewRowData } from '@/components/Table/SuperTable/utils/EditRow'
...
const deleteRowData = () => {
  ...
  const activeCell = useActiveCell.activeCell.value
  if(!activeCell) return
  const index = data.findIndex(item => item.id === activeCell.id)
  const newData = deleteRow(data, index)
  useData.updateData(newData)
  emit('update:isMenu', false)
}
