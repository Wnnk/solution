...
export const insertRowBelow = (data: any, id: string, rowData: any): any => {
  let inserted = false
  // 递归查找并插入节点的函数
  const insertInChildren = (data: any[]): any[] => {
    // 遍历当前层级的每个节点
    for (let i = 0; i < data.length; i++) {
      ...
      if (item.id === id) {
        ...
        // 在当前位置插入新行
        newData.splice(i + 1, 0, rowData)
        return newData
      }
      // 如果当前节点有子节点，递归处理子节点
      if (item.children && item.children.length > 0) {
        const updateChildren = insertInChildren(item.children)
        if (inserted) {
          // 如果子节点中已插入，更新当前节点的children并返回
          return [...data.slice(0, i), { ...item, children: updateChildren }, ...data.slice(i + 1)]
        }
      }
    }
    // 没有找到目标节点，返回原数组
...
export const deleteRow = (data: any, id: string): any => {
  ...
  const deleteInChildren = (data: any[]): any[] => {
    // 遍历当前层级的每个节点
    for (let i = 0; i < data.length; i++) {
      const item = data[i]
      ...
      // 如果当前节点有子节点，递归处理子节点
      if (item.children && item.children.length > 0) {
        const updateChildren = deleteInChildren(item.children)
        if (deleted) {
